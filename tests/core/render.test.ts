...
import { renderKnowledgeDebt, renderMermaidMap, renderProgress, renderToday } from '../../src/core/render.js';
...
describe('renderers', () => {
  it('renders today, debt, and mermaid outputs', () => {
    ...
    expect(today).toContain('Snippet: src/auth.ts');
    expect(today).toContain('Question:');
    expect(renderKnowledgeDebt(state)).toContain('Auth boundary');
    expect(renderMermaidMap(state)).toContain('graph TD');
    expect(renderProgress(state)).toContain('Progress map');
  });
});
