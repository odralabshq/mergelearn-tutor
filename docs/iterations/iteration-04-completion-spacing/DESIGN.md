...
- D1: Replace attempted-card sets with per-card lesson evidence derived from live scheduled ReviewEvents. New events count only when `sessionMode` is lesson, `resultClass` is scheduled, and their request is not tombstoned; evidence-only retries and revisits never raise completion. Legacy events count only when their containing session is lesson mode. Do not add a new evidence file, mastery model, score, or runtime grader.
