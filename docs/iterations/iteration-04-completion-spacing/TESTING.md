...
| Live-event seam | `sessionHistory.test.ts` | Undone request tombstones, evidence-only revisits, and non-lesson planned sessions never count; live scheduled lesson events do; legacy lesson events remain compatible. |
