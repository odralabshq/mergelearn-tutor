...
- D4: `canonicalUrl` must parse as absolute HTTPS, contain no username/password, and have no fragment. Persist `URL.toString()` after default-port removal. Query strings are preserved as authored without interpretation. Validation makes no network request and never claims the target exists or remains current.
...
- D9: Do not add prose-based warnings for missing transfer, leaked pattern names, unstated tradeoffs, or URL query parameters. Those require brittle inference or add low-value policy. Documentation and the example carry faded-guidance, privacy, and constraint-tradeoff guidance.
