...
import { renderDiffSnippetHtml, diffSnippetCss } from '../core/diffView.js';
import { activeLearningItems } from '../core/planner.js';
...
function cards(state: TutorState): string {
  return activeLearningItems(state).slice(0, 8).map((item) => `<section class="card"><div class="meta">${esc(item.questionPlane.replace(/_/g, ' '))} · ${esc(item.difficulty)} · generation ${item.generation}</div><h3>${esc(item.title)}</h3><p class="path">${esc(item.snippet.path)}</p>${renderDiffSnippetHtml(item.snippet.code)}<p><strong>Question:</strong> ${esc(item.prompt)}</p><details><summary>Explanation if stuck</summary><p>${esc(item.explanationMarkdown)}</p></details></section>`).join('\n');
}
