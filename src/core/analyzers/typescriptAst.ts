...
export function analyzeTypeScriptSource(source: string, fileName = 'snippet.ts'): AstConceptFinding[] {
  ...
  const add = (conceptId: string, reason: string) => found.set(conceptId, found.get(conceptId) ?? reason);

  function visit(node: ts.Node): void {
    if (ts.isInterfaceDeclaration(node)) add('typescript.interfaces', `interface ${node.name.text}`);
    if (ts.isTypeAliasDeclaration(node)) {
      add('typescript.type_aliases', `type alias ${node.name.text}`);
      if (ts.isUnionTypeNode(node.type)) add('typescript.union_types', `union type alias ${node.name.text}`);
      if (node.typeParameters?.length) add('typescript.generics', `generic type alias ${node.name.text}`);
    }
  ...
  visit(sf);
  return [...found.entries()].map(([conceptId, reason]) => ({ conceptId, reason }));
}
