import type { CardQualityResult, CardQualityVerdict, LearningItem } from './types.js';
import { clamp } from './util.js';

export function evaluateCardQuality(item: LearningItem, existing: LearningItem[] = []): CardQualityResult {
  const scores = {
  ...
  const warnings = warningsFor(item, scores);
...
function evidenceScore(item: LearningItem): number {
  if (item.evidence.length === 0) return 0;
  const withPath = item.evidence.filter((evidence) => evidence.path.trim().length > 0).length;
  const withSnippet = item.evidence.filter((evidence) => evidence.snippet?.trim()).length;
  return round(clamp(0.35 + Math.min(0.35, item.evidence.length * 0.18) + withPath / item.evidence.length * 0.15 + Math.min(0.15, withSnippet * 0.08), 0, 1));
}
...
function specificityScore(item: LearningItem): number {
  ...
  const pathMentioned = prompt.includes(item.snippet.path.toLowerCase()) || item.evidence.some((evidence) => prompt.includes(evidence.path.toLowerCase()));
...
function sourceDiversityScore(item: LearningItem): number {
  const paths = new Set(item.evidence.map((evidence) => evidence.path));
...
function warningsFor(item: LearningItem, scores: CardQualityResult['scores']): string[] {
