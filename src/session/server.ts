...
function renderQualityPanel(quality: CardQualityResult): string {
  const warnings = quality.warnings.length ? `<p class="quality-warnings">${quality.warnings.map(escapeHtml).join(' · ')}</p>` : '<p class="quality-warnings">No deterministic quality warnings.</p>';
  return `<section class="quality-panel quality-${quality.verdict}"><div><p class="label">Quality gate</p><strong>${escapeHtml(quality.verdict.replace('_', ' '))}</strong></div><div class="quality-scores"><span>Evidence ${score(quality.scores.evidence)}</span><span>Answerable ${score(quality.scores.answerability)}</span><span>Specific ${score(quality.scores.specificity)}</span><span>Duplicate risk ${score(quality.scores.duplicateRisk)}</span></div>${warnings}</section>`;
}
