...
function renderSessionHtml(state: TutorState, preferences: UserPreferences): string {
  ...
  const cards = active.slice(0, 8).map((item, index) => `
    <article class="card recall-card" data-item="${escapeHtml(item.id)}">
      <div class="card-topline"><span>Card ${index + 1}</span>${item.courseId ? `<span>course ${escapeHtml(item.courseId)}</span>` : ''}${item.questionId ? '<span>accepted question</span>' : ''}<span>${escapeHtml(item.questionPlane.replace(/_/g, ' '))}</span><span>${escapeHtml(item.difficulty)}</span><span class="card-state">not reviewed</span></div>
